...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i = 0; i < cArray.length; i++) {
			if(isVowel(Character.toLowerCase(cArray[i]))) {
				res++;
				if(i > 0 && isVowel(Character.toLowerCase(cArray[i-1])))	res--;
			}
		}
		if(res > 1 && Character.toLowerCase(cArray[cArray.length-1]) == 'e')	res--;
	    return res;
	}
